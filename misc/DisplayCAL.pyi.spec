...
if sys.platform == "darwin":
    coll_name = appname
    data = os.path.join(dist_dir, "..", "{}.app".format(appname), "Contents", "Resources")
else:
    coll_name = dist_dir
    data = dist_dir
...
# Scripts
if sys.platform == "darwin":
    scripts = get_scripts(excludes=["{}-apply-profiles".format(appname)])
else:
    scripts = get_scripts()
...
a = Analysis(
    [os.path.join("scripts", script) for script, desc in scripts],
    pathex=["."],
    hiddenimports=[],
    hookspath=None,
    excludes=excludes
...
open(dummy, "wb").close()
compile(dummy, '{}.pyc'.format(dummy), dfile="dummy")
pyz = PYZ(TOC([("dummy", "{}.pyc".format(dummy), "PYMODULE")]))
...
if library_zip:
    ...
    libzip.write(
        os.path.join(HOMEPATH, "PyInstaller", "fake", "fake-site{}".format(py_ext)),
        "site{}".format(py_ext)
    )
    libzip.close()
    ...
    a.scripts = list(
        filter(
            lambda name, script, dtype:
            name not in ["pyi_rth_win32comgenpy"] + [script for script, desc in scripts],
            a.scripts
        )
    )
...
with open(lib_bootstrap, "wb") as py_file:
    py_file.write(
        "\n".join([
            "import sys",
            'sys.path[:0] = [sys.exec_prefix + "/{}"]'.format(lib)
        ]).encode("utf-8")
    )
...
for script, desc in scripts:
    if sys.platform == "darwin":
        ...
        icon = "{}.ico".format(script2pywname(script))
    else:
        icon = os.path.join("256x256", "{}.png".format(script))
    tempver_path = winversion.mktempver(
        os.path.join("misc", "winversion.txt"),
    ...
    exes += [
        EXE(
            ...
            filter(lambda name, script, dtype: name == "_pyi_bootstrap", a.scripts) if not pyz.dependencies else a.scripts,  # PYI bootstrap
...
if sys.platform == "darwin":
    iconpth = os.path.join(appname, "theme", "icons", f"{appname}.icns")
    app = BUNDLE(
        coll,
        icon=iconpth,
...
if sys.platform == "darwin":
    for src, dst in [(
        os.path.join(dist_dir, "..", f"{appname}.app"),
        os.path.join(dist_dir, f"{appname}.app")
    )]:
