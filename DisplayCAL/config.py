...
def get_verified_path(cfg_item_name, path=None):
    """Verify and return dir and filename for a path from the user cfg, or a given path."""
    ...
    if which == "user":
        if not os.path.exists(confighome):
            try:
                ...
                print(
                    f"Warning - could not create configuration directory '{confighome}': {exception}"
                )
                return False
        ...
        try:
            ...
            print(
                f"Warning - could not create configuration directory '{config_sys}': {exception}"
            )
            return False
    return True
